...
class LocationAssigner:
    ...
    def __init__(self, educational_geojson_path: str, hospitality_geojson_path: str):
        ...
        """
        self.educational_locations = self._load_educational_locations(educational_geojson_path)
        ...
        for feature in geojson_data['features']:
            ...
            try:
                if location_type_str == 'restaurant':
                    ...
                    continue  # Only use restaurants, bars, and cafes as work locations
    ...
    def generate_random_home_position(self) -> Position:
        ...
        lat = random.uniform(
            self.TOULOUSE_BOUNDS['min_lat'], 
            self.TOULOUSE_BOUNDS['max_lat']
        )
        lon = random.uniform(
            self.TOULOUSE_BOUNDS['min_lon'], 
            self.TOULOUSE_BOUNDS['max_lon']
        )
    ...
    def find_nearest_school(self, home_position: Position, child_age: int) -> Optional[Location]:
        ...
        for school in available_schools:
            distance = LocationAssigner.calculate_distance(home_position, school.position)
    ...
    def assign_random_work_location(self, person: Person) -> Optional[Location]:
        ...
        employment_rate = self.EMPLOYMENT_RATES.get(gender_str, 0.75)  # Default 75% if gender not found
        ...
        for family in families:
            ...
            for child in family.children:
                if child.school_location:
                    ...
                    stats['school_type_distribution'][school_type] = stats['school_type_distribution'].get(school_type, 0) + 1
                else:
            ...
            for adult in family.parents:
                if adult.work_location:
                    ...
                    distance = LocationAssigner.calculate_distance(family.home_position, adult.work_location.position)
                    stats['work_distances'].append(distance)
                    
                    # Count work types
                    work_type = adult.work_location.location_type.value
                    stats['work_type_distribution'][work_type] = stats['work_type_distribution'].get(work_type, 0) + 1
                else:
                    stats['adults_without_work'] += 1
        
        # Calculate average distances
        if stats['school_distances']:
            stats['avg_school_distance'] = round(sum(stats['school_distances']) / len(stats['school_distances']), 2)
        
        if stats['work_distances']:
            stats['avg_work_distance'] = round(sum(stats['work_distances']) / len(stats['work_distances']), 2)
