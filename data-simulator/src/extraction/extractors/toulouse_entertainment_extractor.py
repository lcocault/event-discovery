...
class ToulouseEntertainmentExtractor(ToulouseBaseExtractor):
    ...
    def _might_contain_venues(self, tags: Dict[str, str]) -> bool:
        ...
        return any(
            tag in tags for tag in ["amenity", "building", "leisure", "tourism"]
        ) and (
            tags.get("amenity") in self.amenity_mappings
            or tags.get("building") in self.building_mappings
            or tags.get("leisure") in self.leisure_mappings
            or tags.get("tourism") in self.tourism_mappings
        )
